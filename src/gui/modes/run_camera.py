...
def run_camera(app):
    try:
        current_camera = int(app.selected_camera[-1])  # Camera inițială
    finally:
         current_camera = 0
    ...
    while app.running and not app.stop_event.is_set():  # Verificăm stop_event pentru oprire
        if int(app.selected_camera[-1]) != current_camera:  # Dacă utilizatorul schimbă camera
            print(f"[INFO] Schimbare cameră: {current_camera} -> {app.selected_camera[-1]}")
            cap.release()  # Eliberăm resursele camerei anterioare
            current_camera = int(app.selected_camera[-1]) 
